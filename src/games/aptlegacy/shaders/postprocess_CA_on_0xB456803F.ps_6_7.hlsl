...
float4 main(
  ...
  bool _149 = !((PostProcess.Settings[17].z) >= -1.0f);
  float _153 = (PostProcess.Settings[4].w) * _143.x;
  float _154 = (PostProcess.Settings[4].w) * _143.y;
  float _155 = (PostProcess.Settings[4].w) * _143.z;
  float _156 = _153 + (PostProcess.Settings[4].z);
  ...
  float4 _183 = sExposureScale.Load(int3(0, 0, 0));
  float _189 = _183.x * (Global.Global.c[87].y);
  float _190 = _189 * _177;
  float _191 = _190 * (PostProcess.Settings[14].x);
  float _192 = _189 * _178;
  float _193 = _192 * (PostProcess.Settings[14].y);
  float _194 = _189 * _179;
  float _195 = _194 * (PostProcess.Settings[14].z);
  float _200 = _52 * 2.0f;
  float _201 = _53 * 2.0f;
  float _202 = _200 + -1.0f;
  float _203 = _201 + -1.0f;
  float _206 = (PostProcess.Settings[13].w) * _203;
  float _207 = _202 * _202;
  float _208 = _206 * _206;
  float _209 = _208 + _207;
  float _210 = sqrt(_209);
  float _212 = (PostProcess.Settings[13].x) * _210;
  float _214 = _212 + (PostProcess.Settings[13].y);
  float _215 = saturate(_214);
  float _217 = log2(_215);
  float _218 = _217 * (PostProcess.Settings[13].z);
  float _219 = exp2(_218);
  float _220 = _191 * (PostProcess.Settings[12].x);
  float _221 = _193 * (PostProcess.Settings[12].y);
  float _222 = _195 * (PostProcess.Settings[12].z);
  ...
  float _234 = (PostProcess.Settings[10].w) * 1.1190600395202637f;
  ...
  float _246 = (PostProcess.OffsetWeight[0].x) * 0.07434873282909393f;
  float _247 = _246 * _241;
  float _248 = _246 * _242;
  float _249 = _246 * _243;
  float _251 = _247 + (PostProcess.OffsetWeight[0].y);
  float _252 = _248 + (PostProcess.OffsetWeight[0].y);
  float _253 = _249 + (PostProcess.OffsetWeight[0].y);
  float4 _256 = s3_3D.Sample(s3_3DSampler, float3(_251, _252, _253));
  ...
  float _271 = 0.8936070799827576f / (PostProcess.Settings[10].w);
  ...
  float3 apt_scaled_lut_output = APTApplyPostProcessLUT(
      ...
      1.f.xxx);
  _272 = apt_scaled_lut_output.x;
  _273 = apt_scaled_lut_output.y;
  _274 = apt_scaled_lut_output.z;
  bool _277 = ((User.c[3].x) > 0.0f) && !APTIsPsychoV();
  if (_277) {
    float _279 = (PostProcess.Settings[10].w) * 1.1190600395202637f;
    ...
    int _293 = asint((User.c[3].y));
    int _294 = _293 & 1;
    bool _295 = (_294 == 0);
    if (!_295) {
      bool _312 = !(_289 <= (User.c[4].x));
      if (!_312) {
        float _314 = max(9.999999974752427e-07f, (User.c[4].x));
        float _315 = _289 / _314;
        float _316 = _315 * (User.c[4].y);
        float _317 = _315 * _315;
        float _318 = _317 * _315;
        float _319 = _318 - _315;
        float _320 = (User.c[10].x) * 0.1666666716337204f;
      ...
      bool _416 = !(_290 <= (User.c[4].x));
      if (!_416) {
        float _418 = max(9.999999974752427e-07f, (User.c[4].x));
        float _419 = _290 / _418;
        float _420 = _419 * (User.c[4].y);
        float _421 = _419 * _419;
        float _422 = _421 * _419;
        float _423 = _422 - _419;
        float _424 = (User.c[10].x) * 0.1666666716337204f;
      ...
      bool _520 = !(_291 <= (User.c[4].x));
      if (!_520) {
        float _522 = max(9.999999974752427e-07f, (User.c[4].x));
        float _523 = _291 / _522;
        float _524 = _523 * (User.c[4].y);
        float _525 = _523 * _523;
        float _526 = _525 * _523;
        float _527 = _526 - _523;
        float _528 = (User.c[10].x) * 0.1666666716337204f;
    ...
    if (_640) {
      ...
      if (!_643) {
        ...
        bool _646 = (_645 == 0);
        if (!_646) {
          float _650 = (User.c[5].x) * 0.5f;
          float _651 = _650 + 0.5f;
          bool _652 = (_651 < 0.5f);
          float _653 = (User.c[5].x) * 5.0f;
          float _654 = select(_652, (User.c[5].x), _653);
          bool _655 = (_626 < _627);
          ...
          bool _697 = (_638 == 0.0f);
          if (!_697) {
            float _701 = abs(User.c[5].x);
            float _702 = saturate(_701);
            uint4 _704 = 0u; sVibranceLUT.GetDimensions(0u, _704.x, _704.y, _704.w);
            ...
            if (!_709) {
              ...
              int _715 = _713 & 63;
              int _716 = _714 & 63;
              float4 _718 = sBlueNoiseR8G8.Load(int4(_715, _716, 0, 0));
            ...
            float4 _784 = sVibranceLUT.Sample(sLinearClampSampler, float2(_779, _774));
            float4 _788 = sVibranceLUT.Sample(sLinearClampSampler, float2(_782, _774));
  ...
  float _835 = (User.c[8].x) * _828;
  float _836 = (User.c[8].y) * _829;
  float _837 = (User.c[8].z) * _830;
  float3 apt_perceptual_film_grain = APTApplyPerceptualFilmGrain(
      float3(_835, _836, _837),
      SV_Position.xy);
  float _842 = (PostProcess.Settings[10].z) * TEXCOORD.x;
  float _843 = _842 * (PostProcess.Settings[9].x);
  float _844 = (PostProcess.Settings[10].z) * TEXCOORD.y;
  float _845 = _844 * (PostProcess.Settings[9].y);
  float _848 = _843 + (PostProcess.Settings[9].z);
  float _849 = _845 + (PostProcess.Settings[9].w);
  float4 _852 = s9.Sample(s9Sampler, float2(_848, _849));
  float _856 = dot(float3(_835, _836, _837), float3(0.2125999927520752f, 0.7152000069618225f, 0.0722000002861023f));
  bool _859 = ((PostProcess.Settings[10].x) > 0.0f);
  int _862 = asint((Global.Global.c[1].w));
  int _863 = select(_859, _862, 0);
  float _864 = (PostProcess.Settings[10].z) * SV_Position.x;
  float _865 = (PostProcess.Settings[10].z) * SV_Position.y;
  uint _866 = uint(_864);
  uint _867 = uint(_865);
  int _868 = _866 & 63;
  int _869 = _867 & 63;
  float4 _871 = sBlueNoiseR8G8.Load(int4(_868, _869, _863, 0));
  bool _873 = ((PostProcess.Settings[10].z) < 1.0f);
  if (_873) {
    float _875 = _864 * 0.015625f;
    float _876 = _865 * 0.015625f;
    float _877 = float((uint)_862);
    float _878 = select(_859, _877, 0.0f);
    float4 _880 = sBlueNoiseR8G8.SampleLevel(sLinearWrapSampler, float3(_875, _876, _878), 0.0f);
    float _882 = _871.y - _880.y;
    float _883 = _882 * (PostProcess.Settings[10].z);
  ...
  bool _908 = ((PostProcess.Settings[10].y) > 0.0f);
  ...
  float _923 = (PostProcess.Settings[2].y) - (PostProcess.Settings[2].x);
  float _924 = _923 * _899;
  float _925 = _924 + (PostProcess.Settings[2].x);
  ...
  _933 = apt_film_grain_output.x;
  _934 = apt_film_grain_output.y;
  _935 = apt_film_grain_output.z;
  float _938 = (PostProcess.Settings[17].y) + 2.0f;
  ...
  float _948 = (PostProcess.Settings[17].y) + 1.0f;
  ...
  float _960 = (PostProcess.Settings[17].y) + 1.0f;
  ...
  float _972 = (PostProcess.Settings[17].y) + 1.0f;
  ...
  float3 apt_post_process_output = APTApplyPostProcessToneMap(
      ...
      true);
  float _975 = apt_post_process_output.x;
  float _976 = apt_post_process_output.y;
  float _977 = apt_post_process_output.z;
  ...
  int _1004 = _1002 & 63;
  int _1005 = _1003 & 63;
  float4 _1007 = sBlueNoiseR8.Load(int4(_1004, _1005, _862, 0));
