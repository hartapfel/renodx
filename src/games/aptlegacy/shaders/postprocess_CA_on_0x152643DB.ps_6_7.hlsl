...
float4 main(
  ...
  float4 SV_Target;
  float4 _37 = s14.Sample(s14Sampler, float2(TEXCOORD.x, TEXCOORD.y));
  float4 _43 = sPostProcessFX_MaskLayer.Sample(sLinearClampSampler, float2(TEXCOORD.z, TEXCOORD.w));
  ...
  float _64 = 0.5f - (PostProcess.Settings[19].x);
  float _65 = 0.5f - (PostProcess.Settings[19].y);
  float _66 = _64 + _57;
  float _67 = _65 + _58;
  float _68 = _66 * 2.0f;
  float _69 = _67 * 2.0f;
  float _70 = _68 + -1.0f;
  float _71 = _69 + -1.0f;
  float _75 = _71 * (Global.Global.c[51].y);
  float _76 = abs(_70);
  float _77 = abs(_71);
  float _79 = (PostProcess.Settings[19].z) * 2.0f;
  float _80 = _79 + -1.0f;
  float _81 = _76 - _80;
  float _82 = _77 - _80;
  float _83 = saturate(_81);
  float _84 = saturate(_82);
  float _85 = _83 * (Global.Global.c[51].x);
  float _86 = _85 * _70;
  float _87 = _75 * _84;
  float _88 = _86 * _86;
  float _89 = _87 * _87;
  ...
  float _111 = (PostProcess.Settings[2].w) * _91;
  float _112 = _108 * _108;
  float _113 = _112 * _111;
  float _114 = _113 * _105;
  float _115 = _114 * _110;
  float _117 = (PostProcess.Settings[2].z) * _86;
  float _118 = (PostProcess.Settings[2].z) * _87;
  float _119 = _118 + _58;
  float _120 = _57 - _117;
  ...
  float4 _131 = s0.SampleLevel(sLinearClampSampler, float2(_123, _119), _128);
  float4 _133 = s0.SampleLevel(sLinearClampSampler, float2(_120, _125), _128);
  float4 _135 = s0.SampleLevel(sLinearClampSampler, float2(_57, _58), _128);
  ...
  float4 _142 = s12_bloom.SampleLevel(sLinearClampSampler, float2(_57, _58), 0.0f);
  float4 _148 = s8.Sample(s8Sampler, float2(_59, _60));
  int _154 = asint((User.c[7].z));
  ...
  float _1263;
  if (!_155) {
    bool _159 = !((PostProcess.Settings[17].z) >= -1.0f);
    float _163 = (PostProcess.Settings[4].w) * _148.x;
    float _164 = (PostProcess.Settings[4].w) * _148.y;
    float _165 = (PostProcess.Settings[4].w) * _148.z;
    float _166 = _163 + (PostProcess.Settings[4].z);
    float _167 = _164 + (PostProcess.Settings[4].z);
    float _168 = _165 + (PostProcess.Settings[4].z);
    if (!_159) {
  ...
  [branch]
  if (_155) {
    bool _198 = ((PostProcess.Settings[7].x) > 0.0f);
    if (_198) {
      float _200 = _37.x + TEXCOORD.x;
      float _201 = _47 + TEXCOORD.y;
      float4 _204 = s2.SampleLevel(s2Sampler, float2(_200, _201), 0.0f);
      bool _208 = ((PostProcess.Settings[6].y) == 1.0f);
      if (_208) {
        float4 _211 = s7.Load(int3(0, 0, 0));
        float _216 = _211.x - (Global.Proj.m[0][2].z);
        float _217 = (Global.Proj.m[0][2].w) / _216;
        _222 = _217;
      } else {
        _222 = (PostProcess.Settings[5].x);
      }
      float _226 = _204.x - (Global.Proj.m[0][2].z);
      float _227 = (Global.Proj.m[0][2].w) / _226;
      float _229 = _222 * (PostProcess.Settings[6].w);
      float _230 = _229 + _222;
      float _231 = _222 - _229;
      float _232 = max(_227, _231);
      float _233 = min(_232, _230);
      float _236 = _227 - _233;
      float _237 = (PostProcess.Settings[5].w) * _236;
      float _239 = _233 - (PostProcess.Settings[5].y);
      float _240 = _239 * _227;
      float _241 = _237 / _240;
      float _242 = min(_241, 0.0f);
      float _244 = _229 + 1.0f;
      float _245 = 1.0f / _244;
      float _246 = _242 * (PostProcess.Settings[7].z);
      float _247 = max(0.0f, _241);
      float _250 = (PostProcess.Settings[18].x) * _247;
      ...
      float _258 = -0.0f - (PostProcess.Settings[6].x);
      ...
      float _267 = _266 * (PostProcess.Settings[7].y);
      float _268 = exp2(_267);
      float _269 = _268 * (PostProcess.Settings[7].x);
      ...
      float _280 = (User.c[2].x) * _272;
      float _281 = _280 * _279;
      float _282 = (User.c[2].y) * _273;
      ...
      float4 _291 = s0.SampleLevel(sLinearClampSampler, float2(_289, _290), _128);
      float4 _293 = s0.SampleLevel(sLinearClampSampler, float2(_287, _288), _128);
      float4 _295 = s2.SampleLevel(s2Sampler, float2(_289, _290), 0.0f);
      if (_208) {
        float4 _299 = s7.Load(int3(0, 0, 0));
        float _301 = _299.x - (Global.Proj.m[0][2].z);
        float _302 = (Global.Proj.m[0][2].w) / _301;
        _306 = _302;
      } else {
        _306 = (PostProcess.Settings[5].x);
      }
      float _307 = _295.x - (Global.Proj.m[0][2].z);
      float _308 = (Global.Proj.m[0][2].w) / _307;
      float _309 = _306 * (PostProcess.Settings[6].w);
      float _310 = _309 + _306;
      float _311 = _306 - _309;
      float _312 = max(_308, _311);
      float _313 = min(_312, _310);
      float _314 = _308 - _313;
      float _315 = _314 * (PostProcess.Settings[5].w);
      float _316 = _313 - (PostProcess.Settings[5].y);
      float _317 = _316 * _308;
      float _318 = _315 / _317;
      float _319 = min(_318, 0.0f);
      float _320 = _309 + 1.0f;
      float _321 = 1.0f / _320;
      float _322 = _319 * (PostProcess.Settings[7].z);
      float _323 = max(0.0f, _318);
      float _324 = _323 * (PostProcess.Settings[18].x);
      ...
      float4 _332 = s2.SampleLevel(s2Sampler, float2(_287, _288), 0.0f);
      if (_208) {
        float4 _336 = s7.Load(int3(0, 0, 0));
        float _338 = _336.x - (Global.Proj.m[0][2].z);
        float _339 = (Global.Proj.m[0][2].w) / _338;
        _343 = _339;
      } else {
        _343 = (PostProcess.Settings[5].x);
      }
      float _344 = _332.x - (Global.Proj.m[0][2].z);
      float _345 = (Global.Proj.m[0][2].w) / _344;
      float _346 = _343 * (PostProcess.Settings[6].w);
      float _347 = _346 + _343;
      float _348 = _343 - _346;
      float _349 = max(_345, _348);
      float _350 = min(_349, _347);
      float _351 = _345 - _350;
      float _352 = _351 * (PostProcess.Settings[5].w);
      float _353 = _350 - (PostProcess.Settings[5].y);
      float _354 = _353 * _345;
      float _355 = _352 / _354;
      float _356 = min(_355, 0.0f);
      float _357 = _346 + 1.0f;
      float _358 = 1.0f / _357;
      float _359 = _356 * (PostProcess.Settings[7].z);
      float _360 = max(0.0f, _355);
      float _361 = _360 * (PostProcess.Settings[18].x);
      ...
      float _369 = abs(_331);
      float _370 = _369 / (PostProcess.Settings[6].x);
      ...
      float _376 = abs(_368);
      float _377 = _376 / (PostProcess.Settings[6].x);
    ...
    int _385 = asint((User.c[7].y));
    bool _386 = ((int)_385 > (int)0);
    if (_386) {
      float _388 = _37.x + TEXCOORD.x;
      float _389 = _47 + TEXCOORD.y;
      float4 _392 = s4.Sample(s4Sampler, float2(_388, _389));
      float4 _399 = s5.Sample(s5Sampler, float2(_388, _389));
      float _403 = (PostProcess.Settings[6].x) * _399.x;
      float _407 = _403 * (PostProcess.Settings[7].x);
      float _408 = _403 * (PostProcess.Settings[7].y);
      float _409 = _407 + _388;
      float _410 = _408 + _389;
      float4 _411 = s4.Sample(s4Sampler, float2(_409, _410));
      float4 _413 = s5.Sample(s5Sampler, float2(_409, _410));
      float _415 = _413.x * (PostProcess.Settings[6].x);
      float _416 = abs(_415);
      float _418 = _416 / (PostProcess.Settings[7].w);
      ...
      int _433 = asint((User.c[7].x));
      bool _434 = ((int)_433 > (int)0);
      [branch]
      if (_434) {
        float4 _438 = s7.Sample(s7Sampler, float2(TEXCOORD.x, TEXCOORD.y));
        float _440 = abs(_438.x);
        _533 = _440;
      } else {
        float4 _444 = s2.SampleLevel(s2Sampler, float2(TEXCOORD.x, TEXCOORD.y), 0.0f);
        float _446 = TEXCOORD.x * 2.0f;
        float _447 = TEXCOORD.y * 2.0f;
        float _448 = _446 + -1.0f;
        float _449 = _447 + -1.0f;
        float _470 = (Global.Proj.m[0][8].x) * _448;
        float _471 = mad(_449, (Global.Proj.m[0][8].y), _470);
        float _472 = mad(_444.x, (Global.Proj.m[0][8].z), _471);
        float _473 = _472 + (Global.Proj.m[0][8].w);
        float _474 = (Global.Proj.m[0][9].x) * _448;
        float _475 = mad(_449, (Global.Proj.m[0][9].y), _474);
        float _476 = mad(_444.x, (Global.Proj.m[0][9].z), _475);
        float _477 = _476 + (Global.Proj.m[0][9].w);
        float _478 = (Global.Proj.m[0][10].x) * _448;
        float _479 = mad(_449, (Global.Proj.m[0][10].y), _478);
        float _480 = mad(_444.x, (Global.Proj.m[0][10].z), _479);
        float _481 = _480 + (Global.Proj.m[0][10].w);
        float _482 = (Global.Proj.m[0][11].x) * _448;
        float _483 = mad(_449, (Global.Proj.m[0][11].y), _482);
        float _484 = mad(_444.x, (Global.Proj.m[0][11].z), _483);
        float _485 = _484 + (Global.Proj.m[0][11].w);
        ...
        float4 _497 = s5.Sample(s5Sampler, float2(TEXCOORD.x, TEXCOORD.y));
        float _503 = (PostProcess.Settings[6].w) * (PostProcess.Settings[5].x);
        float _504 = _503 + (PostProcess.Settings[5].x);
        float _505 = (PostProcess.Settings[5].x) - _503;
        float _506 = max(_494, _505);
        float _507 = min(_506, _504);
        float _509 = _494 - _507;
        float _510 = _509 * (PostProcess.Settings[5].w);
        float _512 = _507 - (PostProcess.Settings[5].y);
        float _513 = _512 * _494;
        float _514 = _510 / _513;
        float _515 = min(_514, 0.0f);
        float _518 = _503 + 1.0f;
        float _519 = 1.0f / _518;
        float _520 = (PostProcess.Settings[7].z) * _515;
        float _521 = max(0.0f, _514);
        float _524 = (PostProcess.Settings[18].x) * _521;
      ...
      float _536 = (PostProcess.Settings[6].x) * _533;
      float4 _539 = s4.Sample(s4Sampler, float2(TEXCOORD.x, TEXCOORD.y));
      float _546 = (PostProcess.Settings[7].x) * _536;
      float _547 = (PostProcess.Settings[7].y) * _536;
      float _548 = _546 + TEXCOORD.x;
      float _549 = _547 + TEXCOORD.y;
      float4 _550 = s4.Sample(s4Sampler, float2(_548, _549));
      float4 _554 = s5.Sample(s5Sampler, float2(_548, _549));
  ...
  if (_155) {
    bool _578 = !((PostProcess.Settings[17].z) >= -1.0f);
    float _582 = (PostProcess.Settings[4].w) * _148.x;
    float _583 = (PostProcess.Settings[4].w) * _148.y;
    float _584 = (PostProcess.Settings[4].w) * _148.z;
    float _585 = _582 + (PostProcess.Settings[4].z);
    float _586 = _583 + (PostProcess.Settings[4].z);
    float _587 = _584 + (PostProcess.Settings[4].z);
    if (!_578) {
  ...
  float4 _616 = sExposureScale.Load(int3(0, 0, 0));
  float _622 = _616.x * (Global.Global.c[87].y);
  float _623 = _622 * _610;
  float _624 = _623 * (PostProcess.Settings[14].x);
  float _625 = _622 * _611;
  float _626 = _625 * (PostProcess.Settings[14].y);
  float _627 = _622 * _612;
  float _628 = _627 * (PostProcess.Settings[14].z);
  float _633 = _57 * 2.0f;
  float _634 = _58 * 2.0f;
  float _635 = _633 + -1.0f;
  float _636 = _634 + -1.0f;
  float _639 = (PostProcess.Settings[13].w) * _636;
  float _640 = _635 * _635;
  float _641 = _639 * _639;
  float _642 = _641 + _640;
  float _643 = sqrt(_642);
  float _645 = (PostProcess.Settings[13].x) * _643;
  float _647 = _645 + (PostProcess.Settings[13].y);
  float _648 = saturate(_647);
  float _650 = log2(_648);
  float _651 = _650 * (PostProcess.Settings[13].z);
  float _652 = exp2(_651);
  float _653 = _624 * (PostProcess.Settings[12].x);
  float _654 = _626 * (PostProcess.Settings[12].y);
  float _655 = _628 * (PostProcess.Settings[12].z);
  ...
  float _667 = (PostProcess.Settings[10].w) * 1.1190600395202637f;
  ...
  float _679 = (PostProcess.OffsetWeight[0].x) * 0.07434873282909393f;
  float _680 = _679 * _674;
  float _681 = _679 * _675;
  float _682 = _679 * _676;
  float _684 = _680 + (PostProcess.OffsetWeight[0].y);
  float _685 = _681 + (PostProcess.OffsetWeight[0].y);
  float _686 = _682 + (PostProcess.OffsetWeight[0].y);
  float4 _689 = s3_3D.Sample(s3_3DSampler, float3(_684, _685, _686));
  ...
  float _704 = 0.8936070799827576f / (PostProcess.Settings[10].w);
  ...
  float3 apt_scaled_lut_output = APTApplyPostProcessLUT(
      ...
      1.f.xxx);
  _705 = apt_scaled_lut_output.x;
  _706 = apt_scaled_lut_output.y;
  _707 = apt_scaled_lut_output.z;
  bool _710 = ((User.c[3].x) > 0.0f) && !APTIsPsychoV();
  if (_710) {
    float _712 = (PostProcess.Settings[10].w) * 1.1190600395202637f;
    ...
    int _726 = asint((User.c[3].y));
    int _727 = _726 & 1;
    bool _728 = (_727 == 0);
    if (!_728) {
      bool _745 = !(_722 <= (User.c[4].x));
      if (!_745) {
        float _747 = max(9.999999974752427e-07f, (User.c[4].x));
        float _748 = _722 / _747;
        float _749 = _748 * (User.c[4].y);
        float _750 = _748 * _748;
        float _751 = _750 * _748;
        float _752 = _751 - _748;
        float _753 = (User.c[10].x) * 0.1666666716337204f;
      ...
      bool _849 = !(_723 <= (User.c[4].x));
      if (!_849) {
        float _851 = max(9.999999974752427e-07f, (User.c[4].x));
        float _852 = _723 / _851;
        float _853 = _852 * (User.c[4].y);
        float _854 = _852 * _852;
        float _855 = _854 * _852;
        float _856 = _855 - _852;
        float _857 = (User.c[10].x) * 0.1666666716337204f;
      ...
      bool _953 = !(_724 <= (User.c[4].x));
      if (!_953) {
        float _955 = max(9.999999974752427e-07f, (User.c[4].x));
        float _956 = _724 / _955;
        float _957 = _956 * (User.c[4].y);
        float _958 = _956 * _956;
        float _959 = _958 * _956;
        float _960 = _959 - _956;
        float _961 = (User.c[10].x) * 0.1666666716337204f;
    ...
    if (_1073) {
      ...
      if (!_1076) {
        ...
        bool _1079 = (_1078 == 0);
        if (!_1079) {
          float _1083 = (User.c[5].x) * 0.5f;
          float _1084 = _1083 + 0.5f;
          bool _1085 = (_1084 < 0.5f);
          float _1086 = (User.c[5].x) * 5.0f;
          float _1087 = select(_1085, (User.c[5].x), _1086);
          ...
          if (!_1130) {
            float _1134 = abs(User.c[5].x);
            float _1135 = saturate(_1134);
            uint4 _1137 = 0u; sVibranceLUT.GetDimensions(0u, _1137.x, _1137.y, _1137.w);
            ...
            if (!_1142) {
              ...
              int _1148 = _1146 & 63;
              int _1149 = _1147 & 63;
              float4 _1151 = sBlueNoiseR8G8.Load(int4(_1148, _1149, 0, 0));
            ...
            float4 _1217 = sVibranceLUT.Sample(sLinearClampSampler, float2(_1212, _1207));
            float4 _1221 = sVibranceLUT.Sample(sLinearClampSampler, float2(_1215, _1207));
  ...
  float3 apt_perceptual_film_grain = APTApplyPerceptualFilmGrain(
      float3((User.c[8].x) * _1261, (User.c[8].y) * _1262, (User.c[8].z) * _1263),
      SV_Position.xy);
  float _1270 = apt_perceptual_film_grain.x;
  float _1271 = (PostProcess.Settings[17].y) + 2.0f;
  ...
  float _1281 = (PostProcess.Settings[17].y) + 1.0f;
  float _1282 = _1281 / _1271;
  float _1283 = select(_1278, _1280, _1282);
  float _1284 = apt_perceptual_film_grain.y;
  ...
  float _1294 = (PostProcess.Settings[17].y) + 1.0f;
  float _1295 = _1294 / _1271;
  float _1296 = select(_1291, _1293, _1295);
  float _1297 = apt_perceptual_film_grain.z;
  ...
  float _1307 = (PostProcess.Settings[17].y) + 1.0f;
  ...
  float3 apt_post_process_output = APTApplyPostProcessToneMap(
      ...
      true);
  float _1310 = apt_post_process_output.x;
  float _1311 = apt_post_process_output.y;
  float _1312 = apt_post_process_output.z;
  ...
  int _1339 = asint((Global.Global.c[1].w));
  uint _1340 = uint(SV_Position.x);
  uint _1341 = uint(SV_Position.y);
  int _1342 = _1340 & 63;
  int _1343 = _1341 & 63;
  float4 _1345 = sBlueNoiseR8.Load(int4(_1342, _1343, _1339, 0));
