...
float4 main(
  ...
  bool _562 = ((User_000.UserConstant_Z_000[3].x) > 0.0f) && !APTIsPsychoV();
  if (_562) {
  ...
  float3 apt_perceptual_film_grain = APTApplyPerceptualFilmGrain(
      ...
      SV_Position.xy);
  float _1122 = apt_perceptual_film_grain.x;
  ...
  float _1136 = apt_perceptual_film_grain.y;
  ...
  float _1149 = apt_perceptual_film_grain.z;
  ...
  float3 apt_post_process_output = APTApplyPostProcessToneMap(
      ...
      true);
  float _1162 = apt_post_process_output.x;
  float _1163 = apt_post_process_output.y;
  float _1164 = apt_post_process_output.z;
