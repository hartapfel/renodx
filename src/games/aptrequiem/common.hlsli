...
float3 APTToneMapPsychoV30(float3 color_bt709) {
...
float3 APTApplyLUTBuilderPsychoV(
  ...
  float3 mapped_bt709 = APTToneMapPsychoV30(psychov_input);
  ...
  if (mapped_peak > 1.f && peak_ratio > 1.f) {
    ...
    const float lut_endpoint_luminance = max(
        renodx::color::y::from::BT709(
            APTDecodeLUTLog(1.f.xxx, linear_scale) * post_lut_scale)
            * max(lut_luminance_gain, 0.f),
        1.f);
    const float3 endpoint_input = APTApplyPsychoVInputExtensions(
        lut_endpoint_luminance.xxx);
    const float endpoint_peak = renodx::math::Max(
        APTApplyPsychoVOutputExtensions(
            endpoint_input,
            APTToneMapPsychoV30(endpoint_input)));
