...
enum InputFlags : uint32_t { motion_depth = 1, hudless = 2, hdr_encoded = 4 };
...
struct alignas(8) Inputs {
  ...
  uint64_t textures[3] = {}; // motion RGBA16F, depth R32F, HUD-less output
...
static_assert(sizeof(Inputs) == 272 && offsetof(Inputs, current_camera) == 80);
}
