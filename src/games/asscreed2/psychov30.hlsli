...
float psycho30_ShadowsScalar(
    ...
    float shadows,
    float adapted_anchor_yf) {
  // AC2: the original subtractive curve could produce negative Yf in deep
  // shadows. CopySign later reflected it into positive light; at shadows=2,
  // pow(ratio, 0) also introduced a nonzero black offset. Grade distance below
  // the anchor instead: positive, monotone, black-preserving and unit slope
  // at the anchor for the complete 0..2 control range (UI 0..100).
  if (shadows == 1.f || x <= 0.f || x >= adapted_anchor_yf) return x;
  const float stops = log2(adapted_anchor_yf) - log2(x);
  const float exponent = exp2(1.f - clamp(shadows, 0.f, 2.f));
  const float graded_stops = stops * (1.f + (exponent - 1.f)
      * stops / (stops + PSYCHO30_SHADOW_GRADE_RANGE_STOPS));
  return adapted_anchor_yf * exp2(-graded_stops);
}
...
float3 psycho30_MeanA2ResponseFromPositiveQ(
  ...
  if (source_radius6 > 6.f * PSYCHO30_EPSILON2
    ...
    if (mean_radius6 > PSYCHO30_EPSILON2) {
      ...
      if (hue_restore != 0.f) {
        // Preserve the Legacy response through weight 2 (UI 100), then allow
        // stronger extrapolation up to weight 10 (UI 500).
