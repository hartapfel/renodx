...
- PsychoV can emulate the SDR display response with None, gamma 2.2, or BT.1886 options. Gamma 2.2 is the default; the correction is applied to the scene before a fixed sRGB intermediate encode, matching the native HDR transform's sRGB decoder without affecting Vanilla or separately composited HUD elements.
...
- PsychoV keeps the scene and final HDR output transform fixed at a 203-nit intermediate reference. Confirmed HUD shaders scale their own sRGB-encoded RGB in linear light before native composition, preserving scene contrast beneath translucent UI while independently controlling UI brightness.
...
- Build with `cmake --build --preset vs-x64-release --target resonanceplaguetalelegacy` and confirm `embed/shaders.h` registers 110 shaders: two LUT builders, 102 postprocess variants, five HUD variants, and one output transform.
...
- Select PsychoV-30 and verify peak brightness, paper white, UI brightness, SDR gamma emulation, LUT luminance, exposure, grading gamma, highlights, shadows, contrast, saturation, highlight saturation, blowout, flare, and hue controls independently. When testing UI Brightness, compare a static scene with visible HUD: HUD luminance should change while scene luminance remains stable.
