...
cbuffer BiasCurrentColorConstants : register(b0) {
  ...
  // xy: previous-minus-current jitter in UV space.
...
void main(uint3 dispatch_id : SV_DispatchThreadID) {
  ...
  const bool outside_history = any(previous_uv <= 0.f.xx) || any(previous_uv >= 1.f.xx);
  ...
  const float relative_color_delta = peak_delta / max(max(current_luma, previous_luma), 0.1f);
  const float color_response = saturate(
      (relative_color_delta - g_BiasMaskParams.z)
      / max(g_BiasMaskParams.z * 3.f, 1e-4f));
  ...
  const float rejection = outside_history ? 1.f : max(color_response, depth_response);
