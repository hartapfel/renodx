// Reconstructed from embedded DXIL debug source for PostProcessHDRWithBlur.pso.
// Active feature defines from the original compile command:
//   DISTORTION, MOTION_BLUR, CONTRAST_SATURATION, DEPTH_OF_FIELD,
...
float4 main(
  ...
  float2 vScreenPositionOriginal = vScreenPosition;

#if defined(DISTORTION)
  float4 distortMap = g_DistortionTexture.Sample(g_SamplerLinearClamp, vScreenPosition);
  float2 wobbleOffset = ((2.f * distortMap.xy) - 1.f) * distortMap.z;
  vScreenPosition.xy = clamp(vScreenPosition.xy + wobbleOffset * 0.03f, g_ViewPort.xy, g_ViewPort.zw);
#endif
  ...
  if (viewDepth > 0.005f) {
    float4 lastNsPos = mul(
        g_MotionBlurXform,
        float4(vScreenPosition.x * g_DOFBlurVals.w, vScreenPosition.y * g_DOFBlurVals.w, depth, 1.f));
    lastNsPos.xyz /= lastNsPos.w;

    float2 vel = (vScreenPosition.xy * g_DOFBlurVals.w) - lastNsPos.xy;
    ...
    if (length(vel) > 0.01f) {
      ...
      for (int i = 1; i < numSamples; ++i) {
        float2 offset = clamp(
            (vScreenPosition * g_DOFBlurVals.w) + vel * i * g_MiscParams.w,
            g_ViewPort.xy * g_DOFBlurVals.w,
            g_ViewPort.zw * g_DOFBlurVals.w);
        scene += g_SceneTexture.Sample(g_SamplerLinearClamp, offset).rgb;
      }
      scene /= numSamples;
    }
  }
  ...
  output = lorwin::ApplyPerceptualFilmGrain(output, vScreenPositionOriginal);
